    ...
    def __init__(self, num_tokens, dim, nfeats, depth, heads, dim_head, attn_dropout, ff_dropout, style='col'):
        ...
        for _ in range(depth):
            if self.style == 'colrow':
                self.layers.append(nn.ModuleList([  # residual 用于残差连接
...
class MLP(nn.Module):
    def __init__(self, dims, act=None):
        super().__init__()
        dims_pairs = list(zip(dims[:-1], dims[1:]))  # 两两组合压缩到一块
        layers = []
        for ind, (dim_in, dim_out) in enumerate(dims_pairs):  # 遍历每个维度对，分别作为输入和输出
            is_last = ind >= (len(dims) - 1)  # 判断是否是最后一个维度对
            linear = nn.Linear(dim_in, dim_out)  # 线性层
            layers.append(linear)  # 添加到layers里
        ...
        self.mlp = nn.Sequential(*layers)  # 将刚刚遍历的层展开并转化成seq格式
